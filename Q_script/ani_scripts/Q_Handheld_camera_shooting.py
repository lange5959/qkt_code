...
print os.path.dirname(__file__) + '\Handheld 0626'
...
class Handheld_camera_shooting(QtGui.QDialog):
    ...
    def cam_copyKeyframes(self, cam):
        # loc
        temp_loc = spaceLocator()
        cam_name = cam.name()
        # copy key to loc
        self.copyKeyframes(cam, temp_loc)
        cmds.cutKey(cam_name, animation='objects', clear=1)
        # cam rig
        self.cam_noaim_con(cam)
        # to loc's pos
        loc_p = temp_loc.translate.get()
        print loc_p

        cam_con = cam_name + '_outside_con'

        select(cam_con)
        obj_cam_con = selected()[0]
        print obj_cam_con
        move(obj_cam_con, loc_p, rpr=1)

        cam_com_grp = cam_con + '_grp'
        select(cam_com_grp)
        cam_com_grp = selected()[0]
        cam_com_grp.translate.set([0, 0, 0])
        cam_com_grp.rotate.set([0, 0, 0])


        self.copyKeyframes(temp_loc, obj_cam_con)
        ...
        # obj_cam_con.sx.setKey(v=loc_p[0], t=t[i])
