...
class MainWidget(QtWidgets.QMainWindow):
    ...
    def populate3(self, pid=None):
        ...
        info = self.sql_back(search, inputfiled)
        file_path = info[0][3]
        # print search
        payload = {inputfiled: search, 'key': 0}
        r = requests.get('http://192.168.0.34/phpconn/find.php', params=payload)
        for i in r.json():
            ...
            try:
                ...
                if screenshot:
                    icon = QtGui.QIcon(screenshot)
                    item.setIcon(icon)
                    item.setToolTip(i['file_path'])
                    self.library_sql[name] = i['file_path']
                self.listWidget.setIconSize(QtCore.QSize(size, size))
                buffer = 12
                self.listWidget.setGridSize(QtCore.QSize(size + buffer, size + buffer))
            except:
    ...
    def info_populate(self, item):
        ...
        file_id = item_name.split("_")[-1]
        search_info = file_id
        inputfiled = 'id'
        info_list = self.sql_back(search_info, inputfiled)
        # print info_list[0][-1]
        info = info_list[0][-1]
        file_path = info_list[0][3]
        print '>'*100
        print info_list
        print '<'*100
        self.info_label.setText("<b>Description: %s<br>Name: %s <br>Path: %s</b><br>File Size: Null"\
                                % (info, item_name, file_path))
    ...
    def listItemRightClicked(self, QPos):
        ...
        menu_item_open = self.listMenu.addAction("打开文件in maya")
        ...
        menu_item_open.triggered.connect(self.menuItemClicked_open)
    ...
    def menuItemClicked_open(self):
        ...
        """
        file_path = str(self.listWidget.currentItem().toolTip())
        cmds.file(file_path, open=1, f=1, iv=1)
    ...
    def sql_back(self, search_info, inputfiled):
        ...
        r = requests.get('http://192.168.0.34/phpconn/find.php', params=payload)
        for i in r.json():
            sub_list = (i['pid'], i['name'], i['id'], i['file_path'], i['description'])
    ...
    def sql_add(pid=None, name=None, pic_path=None, file_path=None, description=None, project_name=None,
        ...
        payload = {'id': hot, 'name': name, 'picture_path': pic_path, 'file_name': file_path,
    ...
    def inset_sql(self):
        ...
        picture_path = name_file.split('.')[0]
        picture_path = picture_path + '.jpg'
        pic_path = os.path.dirname(file_path) + '/' + picture_path

        description = self.description_lineedit.text()
        project_name = self.project_combox.currentText()
        face_num = "5000"
        producer = "刘德华"
        payload = {'id': current_pid, 'name': name, 'picture_path': pic_path, 'file_name': file_path,
                   'description': description,
                   'project_name': project_name, 'face_num': face_num, 'producer': producer}
        r = requests.get('http://192.168.0.34/phpconn/add.php', params=payload)
