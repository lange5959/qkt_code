...
class MainForm(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.ships = ships.ShipContainer(QString("ships_dog.dat"))
    ...
    def populateTree(self, selectedShip=None):
        ...
        self.treeWidget.setColumnCount(2)  # 两列
        self.treeWidget.setHeaderLabels(["Country/Owner/Name", "TEU"])  # 标题
        self.treeWidget.setItemsExpandable(True)  # 设置宽度可以拓展？
        parentFromCountry = {}  # 防止重复添加<国家>
        parentFromCountryOwner = {}  # 防止重复添加<Owner>
        for ship in self.ships.inCountryOwnerOrder():
            # print ship.country
            ancestor = parentFromCountry.get(ship.country)
            if ancestor is None:
                ancestor = QTreeWidgetItem(self.treeWidget,  # treeWidget -- country
                                           [ship.country])
                parentFromCountry[ship.country] = ancestor  # 防止重复添加<国家>
            countryowner = ship.country + "/" + ship.owner  # China/Wang , 不同国家的人有可能重名（这里相当于加了一个命名空间）
            parent = parentFromCountryOwner.get(countryowner)
            if parent is None:
                parent = QTreeWidgetItem(ancestor, [ship.owner])  # country -- owner
                parentFromCountryOwner[countryowner] = parent
            item = QTreeWidgetItem(parent, [ship.name,
                                   QString("%L1").arg(ship.teu)])  # owner -- teu
